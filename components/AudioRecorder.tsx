...
const AudioRecorder: React.FC<AudioRecorderProps> = ({ status, setStatus, onAudioAvailable }) => {
  ...
  const handleStop = async () => {
    ...
    // Status will be set to Processing by the parent component via onAudioAvailable
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4">
      ...
      <h2 className="text-2xl font-semibold text-slate-700 mb-2">
        {isRecording ? recordingText : 'Ready to Start'}
      </h2>
      <p className="text-slate-500 mb-6 text-center">
        {isRecording ? recordingSubtext : 'Click a button below to record your dictation or upload an audio file.'}
      </p>
          ...
          <span className="text-slate-500 font-semibold">OR</span>
           ...
           <input 
            ...
            <button
                ...
                className="flex items-center justify-center gap-2 bg-slate-600 text-white font-bold py-3 px-8 rounded-full hover:bg-slate-700 focus:outline-none focus:ring-4 focus:ring-slate-300 transition-all duration-300 ease-in-out transform hover:scale-105 shadow-lg"
